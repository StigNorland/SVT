"""Asymmetric closed Y-junction with L_perp via semi-implicit time stepping.

Status: prototype
Problem type: static
Nondimensionalisation: xi = 1, background density rho0 = 1, longitudinal speed c = 1
Primary observables: total energy (LogSE + L_perp), residual norm, min_density
Primary role: removes the explicit-Euler stiffness wall on L_perp so we can
              reach the paper's quoted lambda = 2000 regime.  Uses FFT-based
              semi-implicit step with `J_perp ~ lambda * (-Laplacian)^2` as
              the implicit preconditioner.
Key limitation: FFT step assumes periodic boundaries; boundary anchor is
                re-applied after each step.  Implicit Jacobian approximation
                is coarse (diagonal in Fourier, no psi-dependence), so for
                accurate dynamics finer methods are needed.  For static
                relaxation, sufficient to find minima.
"""
...
from lperp_implicit_helpers import fourier_k_squared, semi_implicit_step
...
SCRIPT_METADATA = ScriptMetadata(
    ...
    diagnostics=("energy_monotonicity", "residual_norm", "boundary_anchor", "implicit_lperp"),
    issue_refs=("#13",),
    limitations=(
        "Approximate implicit Jacobian: lambda * (-Laplacian)^2, ignoring psi-dependence.",
        "FFT solver assumes periodic BC; boundary anchor reapplied after each step.",
        "Only asymmetric closed Y skeleton supported.",
    ),
)
...
def relax(
    ...
    k_sq = fourier_k_squared(psi.shape, cfg.grid.spacing)
    ...
    for step in range(1, controls.max_steps + 1):
        ...
        candidate = semi_implicit_step(psi, gradient, step_size, lperp.lambda_perp, k_sq)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Asym closed Y with L_perp + semi-implicit FFT stepping.")
    ...
    parser.add_argument("--lambda-perp", type=float, default=10.0)
