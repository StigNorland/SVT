"""Static closed-topology Y-junction prototype: theta-graph with two Y-nodes.
...
Primary role: closed-topology analogue of the open three-prong Y, designed to
              remove the log(hw) F divergence observed in the open geometry.
              Three vortex filaments follow great-circle-like arcs in meridian
              planes at 120 deg apart, joining at two Y-nodes on the z-axis at
              +-node_radius.
Key limitation: this is one specific closed configuration (the theta-graph) and
                is not the (2,3) trefoil knot of trefoil_breather_static.py.
                Both compact-closed candidates can now be measured with the
                Y-junction observables script; comparison is part of the next
                piece.
"""
...
SCRIPT_METADATA = ScriptMetadata(
    ...
    issue_refs=("#13",),
    limitations=(
        "Uses the theta-graph closed topology (two Y-nodes joined by three arcs in meridian planes), not the (2,3) trefoil knot.",
        "Initial condition uses the product-vortex ansatz with one azimuthal winding per arc; no extra colour offset.",
        "N_Y / F extraction will need an arc-aware variant of the open-Y extractor; not implemented in this script.",
    ),
)


@dataclass(frozen=True)
class YJunctionClosedConfig:
    n: int = 48
...
def coordinate_grid(cfg: YJunctionClosedConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    dx = cfg.grid.spacing
    axis = (np.arange(cfg.n) + 0.5) * dx - cfg.half_width
    return np.meshgrid(axis, axis, axis, indexing="ij")


def arc_curve(cfg: YJunctionClosedConfig, k: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Sample filament k's arc, returning per-sample (position, e1, e2) frames.

    e1 = in-meridian-plane direction perpendicular to the tangent (curves with
         the filament; corresponds to the azimuthal-angle reference at theta = 0)
    e2 = out-of-meridian-plane direction (constant for the whole arc, equal to
         the azimuthal unit vector at azimuth phi_k = 2 pi k / 3)
    """
    phi = 2.0 * math.pi * k / 3.0
    alpha = np.linspace(0.0, math.pi, cfg.samples_per_arc, endpoint=True)

    sin_a = np.sin(alpha)
    cos_a = np.cos(alpha)
    pos = np.stack(
        (
            cfg.arc_radius * sin_a * math.cos(phi),
            cfg.arc_radius * sin_a * math.sin(phi),
            cfg.node_radius * cos_a,
        ),
        axis=1,
    )
    ...
    tan = np.stack(
        (
            cfg.arc_radius * cos_a * math.cos(phi),
            cfg.arc_radius * cos_a * math.sin(phi),
            -cfg.node_radius * sin_a,
        ),
        axis=1,
    )
    tan_norm = np.linalg.norm(tan, axis=1, keepdims=True)
    tan = tan / np.maximum(tan_norm, 1.0e-12)

    e2 = np.broadcast_to(
        np.array([-math.sin(phi), math.cos(phi), 0.0]),
        (cfg.samples_per_arc, 3),
    ).copy()

    e1 = np.cross(tan, e2)
    e1_norm = np.linalg.norm(e1, axis=1, keepdims=True)
    e1 = e1 / np.maximum(e1_norm, 1.0e-12)
    return pos, e1, e2


def initial_state(cfg: YJunctionClosedConfig) -> np.ndarray:
    """Three-arc product-vortex ansatz on the theta-graph skeleton."""
    ...
    for k in range(3):
        ...
        phase_total = phase_total + theta_k
...
def apply_boundary_anchor(
    ...
    """Pin / blend outer shells to the anchor target.

    For the compact closed configuration the field at the box boundary should
    already be close to the background (|psi| ~ 1).  We still use the
    initial-state anchor for safety so that any residual structure at the
    boundary is preserved through relaxation.
    """
...
def logse_gradient(psi: np.ndarray, cfg: YJunctionClosedConfig) -> np.ndarray:
    rho = np.maximum(np.abs(psi) ** 2, cfg.density_floor)
    return -0.5 * laplacian(psi, cfg.grid.spacing) + cfg.log_pressure * np.log(rho) * psi


def relax(cfg: YJunctionClosedConfig, controls: RelaxationControls) -> tuple[np.ndarray, RunSummary]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Static closed-topology Y-junction prototype (theta-graph).")
