"""(2,3)-trefoil knot with L_perp via semi-implicit time stepping.
...
Primary role: tests whether large lambda L_perp (now reachable via the FFT-based
              semi-implicit step) prevents the curvature-driven dissolution of
              the (2, 3)-trefoil knot that we documented at moderate lambda
              with explicit Euler.
Key limitation: implicit Jacobian approximation is coarse (lambda * (-Laplacian)^2);
                FFT step + Dirichlet boundary anchor are incompatible at boundary.
"""
...
SCRIPT_METADATA = ScriptMetadata(
    ...
    issue_refs=("#13",),
    limitations=(
        "Approximate implicit Jacobian: lambda * (-Laplacian)^2.",
        "FFT solver assumes periodic BC; boundary anchor reapplied after each step.",
        "Trefoil knot initial condition; same dissolution susceptibility as the explicit version.",
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="(2,3)-trefoil + L_perp with semi-implicit FFT stepping.")
