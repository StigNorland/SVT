...
def evaluate_tier(curves, table, res_all, sample, label):
    """Apply the rule battery to the subset `sample` of fitted galaxies."""
    res = {g: res_all[g] for g in sample}
    return {
        "tier": label,
        "n_galaxies": len(res),
...
def run():
    """Fit EVERY SPARC galaxy once; evaluate the rules on three nested
    tiers. The pre-registered primary tier (Q=1, i>=30, N>=10) carries
    the #133 verdicts; the all-175 tier shows no galaxy is hidden."""
    curves = parse_mass_models(SPARC_DIR / "MassModels_Lelli2016c.mrt")
    table = parse_galaxy_table(SPARC_DIR / "SPARC_Lelli2016c.mrt")
    res_all = {gal: run_galaxy(curves[gal]) for gal in sorted(curves)}
    primary = select_sample(curves, table, q_max=1)
    q2 = select_sample(curves, table, q_max=2)
    everything = sorted(curves)
    out = {
        "issue": 133,
        "sample": {"primary": {"q_max": 1, "min_inc": MIN_INC,
        ...
        "tiers": {
            "primary_preregistered":
                evaluate_tier(curves, table, res_all, primary,
                              "Q=1, i>=30, N>=10 (pre-registered)"),
            "robustness_q_le_2":
                evaluate_tier(curves, table, res_all, q2,
                              "Q<=2, i>=30, N>=10"),
            "all_175_no_cuts":
                evaluate_tier(curves, table, res_all, everything,
                              "all 175, no cuts"),
        },
        "per_galaxy": res_all,
    }
    return out, curves, table, primary
...
def make_figures(out, curves, table, sample):
    ...
    # --- vh vs Mbar ------------------------------------------------------
    rc = out["tiers"]["primary_preregistered"]["rule_c_vh_mbar_scaling"]
    ...
    # --- residual quintiles ----------------------------------------------
    rb = out["tiers"]["primary_preregistered"]["rule_b_residual_quintiles"]
...
def write_per_galaxy_tables(out, table, primary):
    """Readable markdown table + CSV covering ALL 175 galaxies, so the
    full record is inspectable on GitHub (no galaxy is hidden by the
    pre-registered cuts)."""
    res = out["per_galaxy"]
    hdr = ("Galaxy,Q,Inc_deg,N,in_primary,Mbar_Msun,vh_kms,vh_lo,vh_hi,"
           "chi2red_ssv,chi2red_nfw,nfw_v200,nfw_c,chi2red_mond,"
           "chi2red_cored_posthoc,cored_vh,cored_rc_kpc,chi2red_flynn")
    csv_lines = [hdr]
    md = [
        "# SPARC per-galaxy results — all 175 galaxies (issue #133)",
        "",
        "Every SPARC galaxy, fitted identically; no galaxy is excluded "
        "from this table. `primary` marks the pre-registered sample "
        "(Q = 1, i ≥ 30°, ≥ 10 points — SPARC's own quality "
        "recommendations); the #133 verdicts are evaluated on that tier "
        "AND on all 175 (receipt: `sparc_halo_fit_receipt.json`, key "
        "`tiers`). χ²ᵣ = reduced χ². The cored column is the flagged "
        "post-hoc exploration. Generated by "
        "`instruments/paper_vi/sparc_halo_fit.py`.",
        "",
        "| Galaxy | Q | i° | N | primary | v_h [km/s] | χ²ᵣ SSV | "
        "χ²ᵣ NFW | χ²ᵣ MOND | χ²ᵣ cored* | χ²ᵣ Flynn |",
        "|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    prim = set(primary)
    for gal in sorted(res):
        r, t = res[gal], table[gal]
        mbar = 1e9 * (UPS_DISK * t["L36"] + 1.33 * t["MHI"])
        star = "yes" if gal in prim else ""
        md.append(
            f"| {gal} | {t['Q']} | {t['Inc']:.0f} | {r['N']} | {star} | "
            f"{r['vh']:.1f} ({r['vh_lo']:.1f}–{r['vh_hi']:.1f}) | "
            f"{r['chi2red_ssv']:.2f} | {r['chi2red_nfw']:.2f} | "
            f"{r['chi2red_mond']:.2f} | {r['chi2red_cored']:.2f} | "
            f"{r['chi2red_flynn']:.2f} |")
        csv_lines.append(
            f"{gal},{t['Q']},{t['Inc']},{r['N']},{int(gal in prim)},"
            f"{mbar:.4e},{r['vh']:.2f},{r['vh_lo']:.2f},{r['vh_hi']:.2f},"
            f"{r['chi2red_ssv']:.3f},{r['chi2red_nfw']:.3f},"
            f"{r['nfw_v200']:.1f},{r['nfw_c']:.2f},"
            f"{r['chi2red_mond']:.3f},{r['chi2red_cored']:.3f},"
            f"{r['posthoc_cored_vh']:.1f},{r['posthoc_cored_rc']:.2f},"
            f"{r['chi2red_flynn']:.3f}")
    (RESULTS / "sparc-per-galaxy-results.md").write_text(
        "\n".join(md) + "\n", encoding="utf-8")
    (RESULTS / "sparc_per_galaxy_results.csv").write_text(
        "\n".join(csv_lines) + "\n", encoding="utf-8")


def _print_tier(name, tier):
    ra = tier["rule_a_competitiveness"]
    rb = tier["rule_b_residual_quintiles"]
    rc = tier["rule_c_vh_mbar_scaling"]
    ph = tier["posthoc_cored_halo"]
    print(f"--- {name}: {tier['n_galaxies']} galaxies "
          f"[{tier['tier']}]")
    print(f"  rule (a) [{ra['verdict']}]: median chi2red SSV(1p) = "
          ...
          f"SSV BIC-wins {ra['bic_win_fraction']:.0%}")
    print(f"  rule (b) [{rb['verdict']}]: quintiles "
          + ", ".join(f"{x:+.3f}" for x in
                      rb["median_frac_residual_quintiles"]))
    print(f"  rule (c) [{rc['verdict']}]: a = {rc['slope']:.3f} ± "
          f"{rc['slope_err']:.3f} ({rc['n_galaxies']} constrained, "
          f"scatter {rc['scatter_dex']:.2f} dex)")
    cm = tier["comparison_rows"]
    print(f"  MOND/RAR {cm['median_chi2red_mond']:.2f}; Flynn "
          f"{cm['median_chi2red_flynn']:.2f}; post-hoc cored "
          f"{ph['median_chi2red']:.2f} (BIC-wins "
          f"{ph['bic_win_fraction_vs_nfw']:.0%}, median r_c "
          f"{ph['median_rc_kpc']:.1f} kpc)")


def main():
    out, curves, table, primary = run()
    RESULTS.mkdir(parents=True, exist_ok=True)
    dest = RESULTS / "sparc_halo_fit_receipt.json"
    dest.write_text(json.dumps(out, indent=1))
    write_per_galaxy_tables(out, table, primary)
    make_figures(out, curves, table, primary)
    print(f"fitted ALL {out['sample']['n_total_sparc']} SPARC galaxies; "
          f"per-galaxy tables -> sparc-per-galaxy-results.md / .csv")
    for key, name in (("primary_preregistered", "PRIMARY (pre-registered)"),
                      ("robustness_q_le_2", "ROBUSTNESS"),
                      ("all_175_no_cuts", "ALL GALAXIES")):
        _print_tier(name, out["tiers"][key])
