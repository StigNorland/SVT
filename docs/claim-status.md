...
| `Q_p` / `alpha_G` | **Blocked, with mechanism (formally divergent integral).** The outer deficit decays as `r^(-1.8)..(r^(-2.9)` (LogSE algebraic tail); for `Q_p = ∫r²·deficit dr` to converge, need s<−3. The integral **diverges** → no box size yields a finite `Q_p`. The 60–78% box swings are a symptom, not the cause. Unblock requires deriving the carrier operator `C_Q` that suppresses the 1/r² tail, or hard-BC relaxation. NOT a calibration or resolution problem. | [#98](https://github.com/StigNorland/SVT/issues/98) |
