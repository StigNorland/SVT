...
| `Q_p` / `alpha_G` | **Blocked, with mechanism.** The static far-field `Q_p` estimators are not box-converged (deficit-volume spreads 60-78% across `hw5..8`, trend reverses by ansatz) and are dominated by a box-filling/boundary background pedestal (pedestal fraction ~1.0-1.4), not the localized source. Not a calibration (`C_Q`) problem; needs a pedestal-subtracted source isolation or `source << box`. No raw far-field scalar is `alpha_G`. | [#98](https://github.com/StigNorland/SVT/issues/98) |
