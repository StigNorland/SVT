...
$(\ell,\mathcal{O})$, not absent from the medium. The first quantity of \S5.7 retains it;
the second loses it.
...
For the phase-disorder channel, take $I_\phi$ to count the per-mode von Neumann entropies of
...
on the zero-temperature ground state ($n_k = 0$ for all $k$).
...
in \S5.3. Second, the reduction is exact at the level of the phonon channel and serves as a
template: an analogous construction in each remaining channel (with the appropriate mode
basis for $I_\rho$, $I_\mathcal{V}$, $I_\mathcal{K}$, $I_\mathcal{C}$) would reduce $W[\Psi]$
to the full thermodynamic entropy under the union of those coarse-grainings. The general
derivation across all channels remains open; the phonon channel shows the program is not
vacuous.
...
\begin{enumerate}
  \item define the remaining $I_i$ channels in $W[\Psi]$ as computable dimensionless
  functionals (the $I_\phi$ form of \S\ref{sec:phononreduction} provides a template),
  \item fix the remaining coefficients $\alpha_\rho, \alpha_\mathcal{V}, \alpha_\mathcal{K},
  \alpha_\mathcal{C}$ by analogous calibration to known thermodynamic limits,
  \item extend the phonon-channel reduction of \S\ref{sec:phononreduction} to the remaining
  channels, completing the demonstration that wake-complexity reduces to thermodynamic
  entropy under the union of channel-wise coarse-grainings,
