...
claim that time is local change rather than a background dimension. Section 4 develops the
idea that events write wake structure into the medium. Section 5 reframes entropy as
wake-complexity. Section 6 distinguishes formal time reversal in equations from physical
reversal in the medium. Section 7 treats time-travel paradoxes as category errors. Section 8
closes with the remaining open technical questions.
...
In this paper, an ``event'' means a localized update of that structure. Examples include a
vortex displacement, the emission or scattering of phonons, the excitation of Kelvin waves, or
a reconnection. A reconnection is especially clear because it changes the connectivity of
vortex filaments and converts localized order into distributed excitations. But it is not the
only possible event, and it is not used here as a primitive counter for entropy.
...
\subsection{Ordering Without a Block Universe}

The claim that only the present state physically exists does not deny ordering. Earlier and
later remain meaningful because wake structure is asymmetric. A present configuration can
contain records of a previous event: outgoing phases, displaced vortex geometry, thermalized
phonons, and modified reconnection opportunities. The inverse relation is not generally true.
The pre-event configuration does not contain the later wake as an already-existing region.

Thus SSV keeps causal order while rejecting the reification of the past as a second place. The
ordering relation is carried by structure in $\mathcal{P}(t)$, not by an external archive of
completed moments.
...
A usable functional should respond to the parts of $\Psi$ that carry persistent wake structure.
One possible schematic decomposition is
\begin{equation}
  W[\Psi] =
  \alpha_{\phi} I_{\phi}
  + \alpha_{\rho} I_{\rho}
...
The proposed wake formulation is not meant to deny the usefulness of ordinary thermodynamic
entropy. Rather, it proposes a deeper physical interpretation for it. Standard thermodynamic
entropy is recovered at the macroscopic level by coarse-graining over the fine wake structure of
the medium. On this view, extensivity and additivity are not primitive axioms, but emergent
properties of how wake structure composes across weakly coupled subsystems.
...
The standard expression $S=k_B\ln\Omega$ remains useful. The SSV proposal changes the
interpretation of the microstates being counted. Instead of treating $\Omega$ as an abstract
set of hidden possibilities beneath a macrostate, $\Omega_{\text{wake}}$ counts physically
distinct present wake configurations compatible with the same coarse description.
...
to run backward in the same sense. The SSV claim is that this inference is invalid. Formal
reversal is a property of a mathematical description. Physical reversal is a claim about what
operations the medium can actually perform.
