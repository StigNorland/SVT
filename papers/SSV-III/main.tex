...
\paragraph{The audit.}  The SSV master dynamics --- LogSE with the
...
potential}: it carries no dissipation.  The candidate non-conservative
...
\textbf{Branch B is dead as the equations are written.}  Reviving it would require
\emph{adding} a named non-conservative term to the functional --- which
would make SSV a substrate cousin of objective-collapse models --- and that
addition would then be the engine of irreversibility, with the wake
bookkeeping of this paper as its description.
...
\paragraph{What the gravity sector now asks of this paper.}  The same
audit prices a demand that arrived from outside the time sector.  The
\ssvissue{129} token-tax computation (H-BILATERAL S0) showed that the
bilateral availability mechanism --- the surviving candidate for gravity's
spatial sector after the \ssvissue{129}/\ssvissue{138} nulls --- delivers
its $\gamma = 1$ doubling \emph{only} if interaction partners'
microscopic busy-schedules are statistically independent despite their
common mean loading (a smooth common-cause loading gives $\gamma = 0$,
the data-killed half-deflection; Cassini bounds the schedule correlation
at $c \lesssim 5\times10^{-3}$), and it located the only candidate source
of such schedules in \emph{this} paper's update accounting.  The present
computation makes the price explicit: the dynamics audited above is
conservative, T-symmetric, and deterministic --- it contains no
stochastic update schedules.  Whatever supplies them is \emph{added}
...
The status of the paper's central claims after the \ssvissue{137} computation: the
ontology (time as local change; the past as present wake) is unchanged and remains a
philosophical stance rather than a result; the dynamical substrate is \emph{computed} ---
conservative, T-symmetric, with branch B (fundamental irreversibility from reconnection
dissipation) dead as the equations are written; the entropy reframing survives as an
honest coarse-graining with named channels; and the paper's earlier suggestion of
per-event reversal fragility is corrected by measurement (\S\ref{sec:forkcomputed}).
