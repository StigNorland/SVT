...
basis for $I_\mathcal{V}$, $I_\mathcal{K}$, $I_\mathcal{C}$, and a non-phonon $I_\rho$) would
reduce $W[\Psi]$ to the full thermodynamic entropy under the union of those coarse-grainings.
The next subsection carries out the same reduction on a genuinely independent channel, the
Kelvin sector.
...
\begin{enumerate}
  \item define the remaining $I_i$ channels ($I_\mathcal{V}$, $I_\mathcal{C}$, and a
  non-phonon $I_\rho$) as computable dimensionless functionals, following the templates of
  \S\ref{sec:phononreduction} and \S\ref{sec:kelvinreduction},
  \item fix the remaining coefficients $\alpha_\mathcal{V}, \alpha_\mathcal{C}, \alpha_\rho$
  by analogous calibration to known thermodynamic or counting limits,
  \item extend the channel-wise reductions of \S\ref{sec:phononreduction} and
  \S\ref{sec:kelvinreduction} to the remaining channels, completing the demonstration that
  wake-complexity reduces to thermodynamic entropy under the union of channel-wise
  coarse-grainings,
