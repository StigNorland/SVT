...
\begin{enumerate}
  \item define the one remaining channel, a non-phonon $I_\rho$ for solitonic and
  defect-density structure, as a computable dimensionless functional, following the
  configurational template of \S\ref{sec:tanglereduction},
  \item fix the remaining coefficient $\alpha_\rho$ by analogous calibration to a known
  counting limit,
  \item extend the channel-wise reductions of
  \S\ref{sec:phononreduction}--\S\ref{sec:correlationreduction} to the non-phonon density
  channel, completing the demonstration that wake-complexity reduces to thermodynamic entropy
  under the union of channel-wise coarse-grainings,
