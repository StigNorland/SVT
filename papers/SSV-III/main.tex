...
$S = k_B \ln\Omega_{\text{wake}}$ form of \S5.2.
...
vortex channel is therefore the cleanest instance of the interpretation advanced in \S5.2,
where $\Omega_{\text{wake}}$ counts physically distinct present configurations rather than
abstract hidden possibilities beneath a macrostate.
...
\begin{enumerate}
  \item define the remaining $I_i$ channels ($I_\mathcal{C}$ and a non-phonon $I_\rho$) as
  computable dimensionless functionals, following the templates of
  \S\ref{sec:phononreduction}--\S\ref{sec:tanglereduction},
  \item fix the remaining coefficients $\alpha_\mathcal{C}$ and $\alpha_\rho$ by analogous
  calibration to known thermodynamic or counting limits,
  \item extend the channel-wise reductions of \S\ref{sec:phononreduction},
  \S\ref{sec:kelvinreduction}, and \S\ref{sec:tanglereduction} to the remaining two channels,
  completing the demonstration that wake-complexity reduces to thermodynamic entropy under
  the union of channel-wise coarse-grainings,
