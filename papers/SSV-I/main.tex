...
Two empirical inputs are used, $m_e$ and $\alpha$.  The predictions match the
observed lepton and light-hadron spectrum at the $\lesssim 1.5\%$ level, with
explicit dependence on the regularisation cutoff documented in the body.  The
charged-lepton mass ratios (muon, tau) are numerical coincidences rather than
derived rungs, and the proton's charge-to-gravitational-coupling map is open.
Section~\ref{sec:claim-status} tags every quantitative claim as
\textit{derived}, \textit{structural}, \textit{candidate} or \textit{ansatz}.
\end{abstract}
