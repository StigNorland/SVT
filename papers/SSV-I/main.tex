...
$\lesssim 1\%$ level under the present effective theory, with explicit
dependence on the regularisation cutoff $R$ and the meridional domain size
$L_{\rm box}$ documented in the body.  The open problems for Paper~II are:
thin-ring analytic closure of the muon eigenfrequency, full 3D minimisation
for the proton form factor (including a cutoff-stability scan for $F(R)$),
...
\begin{equation}
  ...
  \label{eq:muon-mass}
\end{equation}
is a dynamically motivated rung with an identified mechanism (core-breathing
$\oplus$ Kelvin coupling through $\mathcal{L}_\perp$), a verified selection
rule (driven-response scans), and a numerical eigenfrequency bracket that
contains the target value.  A bracket is not a sharp prediction; the
thin-ring analytic closure deferred to Paper~II is what would turn the
muon identification into a falsifiable number.
...
\end{equation}
with $N_Y\approx3.007$ (topological crossing factor, Appendix~\ref{app:minimisation})
and $F\approx4.47$ at the straight-vortex cutoff $R=1.18\,\xi$ (grid-converged
within $2.5\%$ between $n=48$ and $n=72$ grids), giving
$N_Y\cdot F\approx13.4$ and $m_p\approx941\,\text{MeV}$ ($0.3\%$ from CODATA
at this cutoff).

A dedicated cutoff-stability scan across $R\in[0.5,3.0]\,\xi$ (Appendix~\ref{app:minimisation})
establishes that $F$ is \emph{not} flat: the logarithmic slope is
$\mathrm{d}\ln F/\mathrm{d}\ln R\approx-0.94$ near $R=1.18\,\xi$, implying a
${\sim}3\%$ systematic on $F$ per $0.3\,\xi$ uncertainty in the cutoff.
The predicted $m_p$ ranges from $930$--$954\,\text{MeV}$ across the $n=48$
and $n=72$ fine-grid states at $R=1.18\,\xi$, and would reach ${\sim}760\,\text{MeV}$
($-19\%$) at $R=1.5\,\xi$.  The $0.3\%$ figure is therefore the
precision at the chosen cutoff; the framework prediction with honest
cutoff uncertainty is $m_p = 930$--$954\,\text{MeV}$ (within ${\sim}1.5\%$
of CODATA).
...
\bottomrule
\end{tabular}
\footnotetext[1]{$F=4.47$ is grid-converged at the inter-strand half-spacing
cutoff $R=1.18\,\xi$ (mean of $n=48$ and $n=72$ straight-vortex extractions);
the range $930$--$954\,\text{MeV}$ spans the two grid levels.
$F$ varies as $\mathrm{d}\ln F/\mathrm{d}\ln R\approx-0.94$ in this region;
the cutoff is physically motivated but not derived, giving a ${\sim}3\%$
systematic per $0.3\,\xi$ uncertainty.}
\end{table}
