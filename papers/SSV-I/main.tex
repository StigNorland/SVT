...
beyond the two empirical inputs $m_e$ and $\alpha$.  Third, the core-scale
chiral-to-longitudinal stiffness ratio $c_\perp/c_s$ identifies $\alpha \approx
1/137$ as a geometric property of the medium (a stiffness ratio, not a second
signal speed: the \ssvissue{138} dispersion computation shows the chiral-shear
term supports no propagating linear mode around the uniform vacuum).
...
is developed as a diagnostic for the muon rung, but the present scalar
operator does not close the identification.  The reduced helicity-basis hit
near $(3/2)\mu_0$ is not Galerkin-robust, a direct $m_\varphi=\pm1$ grid-sector
solve finds no corresponding low branch, and a Volovik-style Berry-phase audit
finds trivial holonomy for the implemented $\mathcal{L}_\perp$ block.  The
scalar muon route is therefore retired rather than carried as a pending
candidate rung.  The two structural escape hatches were subsequently opened and
closed negatively: the CP$^1$/spinor branch (\ssvissue{83}, \ssvissue{87}) was
adopted but gives zero muon bridge amplitude ($|V|=0$, \ssvissue{91}), and the
half-quantum-vortex branch (\ssvissue{94}) carries its $\gamma_B=\pi$ on the
meridional circle (the electron spin-$\tfrac12$, not an azimuthal muon phase) and
is charge-excluded as a free object.  The post-\ssvissue{78} lepton-generation
derivation claim is accordingly retired (\ssvissue{99}), not held open.  For the proton, the candidate static-geometry scaling
$m_p c^2 = N_Y\!\cdot\!F\!\cdot\!\mu_0$ now uses the \ssvissue{77} continuation result
$N_Y\!\cdot\!F\simeq54$ at $(R,a)=(2.5,0.85)\xi$, while the separate
$Q_p/\alpha_G$ map remains open.
...
proton-closure work and is tracked under issue~\ssvissue{30}.
...
\textit{ansatz}, and records the issue~\ssvissue{30} closure ledger for the
candidate-grade branches.  The remaining open problems for future work are
topology-preserving static infrastructure for the proton branch and a
closure-grade dynamic reconnection simulator for Paper~II; the charged-lepton
masses (muon, tau) remain numerical coincidences, every $\gamma_B=\pi$
derivation route having been closed (issues~\ssvissue{91}, \ssvissue{94}).  The previously
headlined $0.3\%$ proton-mass
...
fundamental particles.  The masses that the framework actually \emph{derives}
are topological: the charged pion at $2\mu_0$ (two-winding) and the proton at
$N_Y F\mu_0$ with the node factor $N_Y=3$ fixed by the trefoil crossing number,
where $\mu_0=m_e/\alpha$.  A broader empirical observation---that many light
...
Section~\ref{sec:ladder} presents the derived topological masses---the pion
(analytically, two-winding) and the proton (node factor $N_Y=3$ from the
trefoil crossing number, times the breather form factor)---and records the
broader $\alpha$-harmonic mass clustering as an explicitly-flagged numerical
coincidence rather than a derived spectrum.  Section~\ref{sec:forces} sketches the emergence of forces.
...
framework~\cite{volovik2003} and adopting the logarithmic nonlinearity
introduced by Rosen~\cite{rosen1968} and by Bialynicki-Birula and
Mycielski~\cite{bbm1976}, in the form used in superfluid vacuum
theory by Zloshchastiev~\cite{zloshchastiev2020}, we take the Lagrangian:
\begin{equation}
  \mathcal{L}
...
below.  The opposite branch admits the Gausson of Rosen and of
Bialynicki-Birula and Mycielski, but makes the uniform vacuum modulationally
unstable and cannot support $c_s=c$; it is therefore \textbf{rejected}, and with
it the bright-soliton route to particle saturation.  Earlier versions of this
paper printed the rejected sign; see the claim-status table.
\end{resultbox}
...
\item \textbf{Equilibrium radius (analytic derivation, two structural inputs).}
        ...
        earlier versions of this paper misidentified it.  The combination
        ...
        and the one previously made was wrong.  Numerically,
        ...
        length~\eqref{eq:xi}.  Earlier versions of this paper printed this
        quantity with $\Lambda$ in the numerator and a value of
        $1.9\,m_e^4c^3/\hbar^3$, too large by a factor of about
        $2\times10^4$; see the claim-status table.
...
$0.3\%$ figure that appeared in earlier drafts.  \textbf{Update
(issues~\ssvissue{92} and \ssvissue{77}, 2026-06):} the topological node factor $N_Y=3$ is now
...
\begin{resultbox}[Withdrawn: the symmetric Y-junction]
Earlier versions of this paper described the proton as ``a Y-junction of three
quantized vortex filaments meeting at a central node''.  \textbf{That
description is withdrawn}, on two independent grounds established by the
citation audit (issue~\ssvissue{183}):
...
\emph{observes}.  Two masses are genuine topological derivations: the charged
pion at $2\mu_0$ (the two-winding number, \S\ref{sec:pion}) and the proton, whose
node factor $N_Y=3$ is the trefoil crossing number (\S\ref{app:minimisation},
issue~\ssvissue{92}).  The broader observation --- that many light lepton and hadron
...
\subsection{The proton (derived), and the higher-mass coincidences}

The proton is the second genuine topological derivation:
\begin{equation}
  m_p c^2 = N_Y\cdot F\cdot\mu_0,
  \label{eq:proton-ladder}
\end{equation}
with the node factor $N_Y=3$ \emph{derived} as the trefoil crossing number
...
\begin{table}[h]
\centering
\caption{Particle masses vs.\ CODATA. Only the pion ($2$, two-winding) and the
proton ($N_Y=3$, crossing number) are derived; the muon, kaon and $\rho$ entries
are numerical coincidences (no selecting mechanism, issues~\ssvissue{76}, \ssvissue{91}, \ssvissue{93},
\ssvissue{94}).}
\label{tab:ladder}
\begin{tabular}{lcccc}
\toprule
...
presented as one (issue~\ssvissue{93}).  The two masses the framework genuinely derives
are topological---the pion ($2\mu_0$, two-winding) and the proton node factor
($N_Y=3$, the trefoil crossing number)---and these stand on their own,
independent of any ladder.  The higher half-integer rungs (muon $\tfrac32$,
...
assignment $m_{\mu^\pm}=(3/2)\mu_0$ remains an empirical/candidate rung
pending a changed microscopic ingredient.  Agreement with CODATA at the
...
\textbf{Corrected 2026-07 (\ssvissue{183}).}  Earlier versions printed the
opposite sign of the logarithmic potential, $b\rho_0=m_0c^2/2$ (a spurious
factor $2$), $\xi=\hbar/(m_0c)$, and a ``thermodynamic vs.\ Bogoliubov''
discrepancy together with its resolution.  Because $\rho\mu'(\rho)$ is constant
for a logarithmic chemical potential the two routes are identically the same
quantity, so both the discrepancy and its resolution were artifacts and have
been removed.  The rejected sign made the uniform vacuum modulationally
unstable.  \emph{Cost:} the Gausson does not exist on the adopted branch, so
particles must be topological rather than bright solitons
(\ssvissue{189}). \\
Attribution of the logarithmic potential and the LogSE & \textbf{corrected} &
\textbf{2026-07 (\ssvissue{183}).}  Previously credited to Volovik, whose
Phys.\ Rept.\ \textbf{351}, 195 contains no logarithmic equation of state
(``equation of state'' occurs $0$ times in $71{,}425$ words).  The lineage is
Rosen~\cite{rosen1968} $\to$ Bialynicki-Birula and
Mycielski~\cite{bbm1976} $\to$ Zloshchastiev~\cite{zloshchastiev2020}.  The
...
Vacuum saturation density $\rho_0$ & derived & \textbf{Corrected 2026-07
(\ssvissue{183}).}  Previously printed as
$2\alpha\Lambda m_e^4c^3/\pi^2\hbar^3\approx1.9\,m_e^4c^3/\hbar^3$, with
$\Lambda$ in the numerator and a value inconsistent with its own formula.
Inverting~\eqref{eq:electron-mass} correctly gives
$\approx\ssvRhoZero\,m_e^4c^3/\hbar^3$, smaller by $\sim2\times10^4$. \\
Proton skeleton: single-curve trefoil $T_{2,3}$; $N_Y=3$ as crossing number &
derived & \textbf{Corrected 2026-07 (\ssvissue{183}).}  The former
``Y-junction of three quantized vortex filaments'' is \textbf{withdrawn}: it is
absent from the cited sources and is forbidden in a one-component $U(1)$
condensate, where quantized circulation forces $\sum_i n_i=0$ at any node.  No
number changes --- the computations always used the knot. \\
...
as record.  Earlier versions of \emph{this} paper stated it as an established
result. \\
...
inputs of the theory.  \textbf{Corrected 2026-07 (\ssvissue{183}):}
$\mathcal{E}(r)$ previously applied $\alpha^2$ twice --- once in the
chiral-shear coefficient and again in the stationarity condition --- so as
printed it stationarised at $r^*\approx0.57$, not $1/\alpha$.  The
physics is intact; the presentation was not. \\
...
Proton mass $m_p \approx N_Y \cdot F \cdot \mu_0 \approx 924$--$954$
...
and route-independent.  The node factor $N_Y=3$ is separately derived as
...
\paragraph{Parameter count of the mass sector (issue~\ssvissue{95}).}  Every particle
mass derived here has the form $m_X = N_X\,\mu_0$ with $\mu_0 = m_e/\alpha$ and
$N_X$ a pure number fixed by topology (pion $N=2$, two-winding; proton
$N=N_Y F=3F$, the trefoil crossing number).  Hence the mass \emph{ratios}
$m_X/m_e = N_X/\alpha$ depend only on $\alpha$ and topology --- they are
independent of every vacuum parameter $\{\rho_0, m_0, b, \lambda, \xi\}$, which
either cancel from ratios ($m_0$, the graininess scale) or are fixed by
$c_s=c$, $c_\perp/c=\alpha$ and the $m_e$ normalisation.  The SSV mass sector
therefore has exactly \textbf{two irreducible inputs} --- a scale $m_e$ and the
dimensionless $\alpha$ --- versus the Standard Model's $\sim$19.  $\mu_0$ is not
independently predicted; it \emph{is} $m_e/\alpha$.  Nor is $\alpha=1/137$
predicted: the $R_e$ relation $2\pi\gamma\alpha^2=\ln(8/\alpha)-\tfrac34$ defines
$\gamma(\alpha)$ (a consistency condition), so $\alpha$, identified as the
stiffness ratio $c_\perp/c$, remains the one irreducible dimensionless number.
Record: \ssvfile{mu0-parameter-count-result}.

\paragraph{What changed in this paper.}  Several statements that
appeared in earlier drafts have been re-framed:
...
\begin{itemize}
  ...
  \item the $N_Y$ convergence statement ``independent of grid resolution
    ...
    grids ($24^3$, $48^3$, $72^3$);
  \item three cutoff routes for $N_Y\cdot F$ have been tested and all
    fail: cutoff invariance is falsified ($31.5\times$ spread over
    $R=0.5$--$3\,\xi$); geometric-$R$ extraction fails; and the
    first-principles half-density cutoff $R_{\rm sc}=0.923\,\xi$ gives
    $60\%$ cross-state spread at fixed $R_{\rm sc}$
    (\ssvfile{static-closure-self-consistent-r-result}).
    The dominant cause is the penalty parameters influencing the
    converged state geometry.  A first matched cross-grid test
    ($n^3=96^3$ vs $72^3$ at fixed penalty) is in progress
    (2026-06-02, issue~\ssvissue{30});
  \item the muon identification has been tested by three independent
    null results (Path~B, issue~\ssvissue{73}, issue~\ssvissue{76} Berry-phase audit), and the
    CP$^1$/spinor (\ssvissue{91}) and half-quantum-vortex (\ssvissue{94}) routes are likewise
    closed.  No half-integer Berry holonomy is available, so the muon and the
    higher masses are numerical coincidences, not candidate rungs.
\end{itemize}
...
the knot topology exactly). The node-cost factor and form factor are then
extracted by volume integration in arc-tubes and node-balls
...
grid (\ssvfile{trefoil_gradient_flow_static}), and the node-cost factor is
...
node factor, $N_Y = 3$ (the $(2,3)$-trefoil crossing number), is derived in
...
spread for $N_Y$; see issue~\ssvissue{30}.
...
\begin{resultbox}[Two corrections to this appendix (\ssvissue{183})]
\begin{enumerate}[label=(\roman*)]
  \item Earlier versions gave $C=1/8$ and called it ``a pure geometric
        constant from the elliptic expansion''.  It is neither: the
        coefficient is $3/16$, and the term \textbf{carries a logarithm}, so it
  ...
  \item The additive constant in this appendix's own expansion is
        ...
        over the section).  The claim that this appendix ``recovers
        \eqref{eq:Ekin} at leading order'' was therefore \textbf{false}.
        Equation~\eqref{eq:Ekin} and its $-7/4$ are correct and unchanged; it
        is this appendix that was misdescribed.
\end{enumerate}
