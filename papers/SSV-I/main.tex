...
candidate rung; post-\#78 lepton-generation work requires a new HQV or
spinorial branch.  For the proton, the candidate static-geometry scaling
$m_p c^2 = N_Y\!\cdot\!F\!\cdot\!\mu_0$ now uses the \#77 continuation result
...
\textit{ansatz}, and records the issue~\#30 closure ledger for the
...
derivation route having been closed (issues~\#91, \#94).  The previously
...
\#76, \#91) failed to produce the higher rungs as eigenfrequencies.
...
\begin{itemize}
  \item \textbf{Spin.}
        ...
        issue~\#83; Paper~II).  The internal direction
        ...
        Finkelstein--Rubinstein route: the strand-model import (issue~\#29)
...
(issues~\#92 and \#77, 2026-06):} the topological node factor $N_Y=3$ is now
...
proxy is rejected as non-invariant.  The later \#77 continuation scan upgrades
...
issue~\#92).  The broader observation --- that many light lepton and hadron
...
selects the muon, kaon or $\rho$ values (issues~\#76, \#91, \#94).  We state the
...
mechanism (issues~\#76, \#91, \#93) and are recorded here as a numerical
...
\textbf{CP$^1$/spinor extension (\#83, \#87 B1):} The SU(2)-covariant
$\mathcal{L}_\perp$ in principle allows the bridge by total-$J_\varphi$
conservation.  However, issue~\#91 (2026-06) shows the spin--orbit lock
...
\textbf{rejected (issue~\#94):} the HQV half-winding is meridional --- it links
the core and \emph{is} the electron's spin-$\tfrac12$ (the $\mathbb{Z}_2$ framing
of \S\ref{sec:electron}, \#87 B2) --- not azimuthal, so it cannot supply the
muon's major-ring Berry phase ($A_\varphi=0$, reproducing \#91); and a free HQV
...
CP$^1$/spinor extension (\#83, \#87 B1) showed the bridge is symmetry-allowed
by total $J_\varphi$ conservation, but issue~\#91 (2026-06) established that
the SU(2)-covariant $\mathcal{L}_\perp$ gives zero lock amplitude $|V|=0$
on the IQV electron with uniform $\hat n$: the scalar issue~\#76 null is reproduced.
The last route, a half-quantum vortex (HQV) electron, was examined and rejected
(issue~\#94): its half-winding is meridional --- it \emph{is} the electron's
spin-$\tfrac12$ ($\mathbb{Z}_2$ framing, \#87 B2) --- not azimuthal, so it gives
...
structural setup.  Tracking: issues~\#66, \#73, \#76, \#91, \#94.
\end{gapbox}
...
closed --- Path~B, issue~\#73, the issue~\#76 scalar null, the issue~\#91 spinor
lock ($|V|=0$), and the issue~\#94 half-quantum-vortex audit (wrong circle plus
...
writhe, issue~\#92; Appendix~\ref{app:minimisation}) and $F$ the form factor
extracted from a
...
mechanism for them (issues~\#76, \#91, \#93): they are recorded as numerical
...
are numerical coincidences (no selecting mechanism, issues~\#76, \#91, \#93,
\#94).}
\label{tab:ladder}
\begin{tabular}{lcccc}
\toprule
...
presented as one (issue~\#93).  The two masses the framework genuinely derives
...
and does not produce these as eigenfrequencies (Path~A/B, issues~\#76, \#91),
and the half-quantum-vortex route for the muon was examined and rejected
(issue~\#94: the HQV half-winding is the electron's spin-$\tfrac12$, on the
...
issue~\#73, and issue~\#76 audits show that this branch is not a
...
issue~\#30; it requires a changed microscopic ingredient or a genuinely new
...
issue~\#73 finds no stable low branch near the muon target.  The
Volovik/Berry-phase audit of issue~\#76 shows that the scalar SSV BdG
...
\emph{What remains (2026-06 update):}  The CP$^1$/spinor extension (\#83)
was adopted, and issue~\#91 tested whether the SU(2)-covariant
...
(issue~\#94)}: the HQV half-winding is meridional (it \emph{is} the electron's
spin-$\tfrac12$, \#87 B2), not azimuthal, so $A_\varphi=0$ and it gives no muon
...
\textbf{Update (issues~\#92 and \#77, 2026-06):}  $N_Y$ is the
...
\begin{itemize}
  ...
  \item The \#77 geometry scan gives a candidate combined observable
        $N_Y\!\cdot\!F\simeq54$ at $(R,a)=(2.5,0.85)\xi$, but the separate
        $F$ extraction and its propagation into $Q_p/\alpha_G$ remain
        lower-status bookkeeping for issue~\#98.
\end{itemize}
\end{gapbox}

Following the \#77 continuation scan, the proton assignment has a candidate
...
and generation routes are no longer pending candidate mechanisms after \#78:
the BdG route is retired, Route C/D failed, and any future lepton-generation
claim must pass through the HQV/spinorial decision tracked by issue~\#99.
...
factor $F$ at the inter-strand cutoff remains candidate-grade: issue~\#30
...
(spin-$0$ $\Rightarrow$ uniform $\hat n$, \#87 A1). & --- \\
Lepton spin $s=\tfrac12$ & derived & \textbf{Promoted 2026-06 (\#87 B2)}
from assumed to derived: intrinsic to the CP$^1$/spinor order parameter
(\#83), where a $2\pi$ frame rotation returns $\Psi\to-\Psi$
($\pi_1(SO(3))=\mathbb{Z}_2$).  Supersedes the scalar
Finkelstein--Rubinstein route, which \#29 showed does not deliver the
$\mathbb{Z}_2$ for a scalar field. & \#29,\#83,\#87 \\
Muon mass $m_{\mu^\pm}=(3/2)\mu_0$ ($0.59\%$ from CODATA) &
coincidence & \textbf{Demoted 2026-06} from candidate to
numerical coincidence: five probes (Path~B, \#73, \#76, App.~B potential,
...
\textbf{Re-promoted (\#87 B1)} to TOPOLOGICAL-SECTOR (conditional) under
...
\textbf{Conditional closed (issue~\#91, 2026-06): CLEAN NO for the IQV.}
...
$\gamma_B=0$, issue~\#76 null reproduced. The last route (half-quantum vortex
electron) was \textbf{rejected (issue~\#94):} the HQV half-winding is meridional
(= the electron's spin-$\tfrac12$, \#87 B2), not azimuthal, so $A_\varphi=0$
...
\texttt{hqv-muon-verdict.md}. & \#76,\#87,\#91,\#94 \\
...
($\sim$0.7\%) & coincidence & \textbf{Demoted (issue~\#93, 2026-06):} no
deriving mechanism.  Path~A/B and the issue~\#76/\#91 BdG nulls show the
...
Recorded as a numerical coincidence, not a candidate rung. & \#76,\#91,\#93 \\
Proton mass $m_p \approx N_Y \cdot F \cdot \mu_0 \approx 924$--$954$
MeV ($\sim$1.5\% from CODATA) &
candidate & \textbf{Update (\#77):} the static geometry now has a
...
the trefoil crossing number (\#92; see row below), but the separate $F$
...
\texttt{results/proton/geometry-minimum-result-2026-06-03.md}. & \#77,\#92,\#98 \\
$N_Y = 3$ topological crossing factor & derived &
\textbf{Derived (issue~\#92, 2026-06):} $N_Y$ is the trefoil crossing
...
\texttt{results/proton/ny-trefoil-derivation-result.md}. & \#92 \\
Form factor $F$ and product $N_Y\!\cdot\!F$ & candidate &
The separately reported $F$ value remains bookkeeping-sensitive to the
chosen decomposition and calibration convention.  The promoted \#77 output is
...
documented $Q_p/\alpha_G$ map. & \#77,\#98 \\
...
should not be quoted as derived. & closed \#30 \\
...
\paragraph{Parameter count of the mass sector (issue~\#95).}  Every particle
...
\begin{itemize}
  ...
  \item the muon identification has been tested by three independent
    null results (Path~B, issue~\#73, issue~\#76 Berry-phase audit), and the
    CP$^1$/spinor (\#91) and half-quantum-vortex (\#94) routes are likewise
...
issue~\#30 and \S\ref{sec:claim-status}.  \paragraph{Reference implementation.}
...
issue~\#92; the $3.007$ used as the dilute-limit estimate is that $3$ plus a
...
\paragraph{Topological status of $N_Y$ (settled, issue~\#92).}  $N_Y$ is the
...
the Path~B null, the direct phi-sector BdG null of issue~\#73, and the
Berry-phase audit of issue~\#76.
...
the branch is not basis-robust, issue~\#73 showed that it is not reproduced by
a direct $m_\varphi=\pm1$ grid sector, and issue~\#76 showed that the current
