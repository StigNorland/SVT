...
Together these set the base energy scale $\mu_0 \equiv m_ec^2/\alpha \approx
70\,\text{MeV}$, the energy of a minimal vortex filament segment of the
...
$m_{\pi^\pm} = 2\mu_0$, matching CODATA to $0.34\%$.  The neutron is the
...
$2\mu_0$ (two-winding).  For the proton, only the trefoil factor $N_Y=3$ is
fixed by the crossing number; the full mass $N_YF\mu_0$ remains candidate-grade
because the extracted form factor $F$ is cutoff-dependent, where
$\mu_0=m_e/\alpha$.  A broader empirical observation---that many light
hadron and lepton masses cluster near half-integer multiples of $\mu_0$ to
...
The proton mass below is written as $m_p c^2 = N_Y\,F\,\mu_0$.  $N_Y=3$ is the
...
value $938.272\,\text{MeV}$ --- a range with a documented cutoff systematic,
...
$F$ extraction or the static-gravity map closure-grade; $\mu_0=m_e/\alpha$
...
\begin{itemize}
  ...
  \item \textbf{Mass.}
        ...
        \end{equation}
        where $\mu_0\equiv m_e/\alpha\approx70\,\text{MeV}$ is the flux-tube
        mass scale.  The trefoil factor has topological value $N_Y=3$ from the
        ...
        the trefoil reference energy $N_Y\mu_0$.  It can be written explicitly as
        \begin{equation}
...
\emph{observes}.  The charged-pion mass at $2\mu_0$ is a genuine topological
...
$\mu_0=m_e/\alpha$ to $\lesssim0.7\%$ --- is a \emph{numerical coincidence}, not
...
empirical pattern first because it motivates the scale $\mu_0$, then give the two
derivations, then record the coincidental masses honestly.

\subsection{The empirical clustering and the scale $\mu_0$}
...
selection rule is reproduced.  The muon rung $(3/2)\mu_0$ therefore remains
...
$(3/2)\mu_0$ is therefore a \textbf{numerical coincidence} with no surviving
derivation; $m_{\pi^\pm}=2\mu_0$ stands (electron integer-winding).
\end{gapbox}
...
\item \textbf{Issue~\ssvissue{76} (Berry-phase audit):} Volovik's topological CdGM
  ...
  \ssvfile{volovik-berry-phase-issue76-tasks1-4}.
\end{itemize}
The muon assignment $m_{\mu^\pm}=\frac{3}{2}\mu_0$ is a \textbf{numerical
...
other is the pion $m_{\pi^\pm}=2\mu_0$), but it is a \textbf{numerical
...
($-0.9\%$ to $+1.6\%$ of the CODATA value $938.272$ MeV).  $F$ is cutoff-dependent
...
($m_{K^\pm}=493.677\,\text{MeV}=7.05\,\mu_0$, $0.71\%$) and the $\rho(770)$
($11.07\,\mu_0$, $0.65\%$) sit close to half-integer multiples of $\mu_0$, but
...
$p$       & $N_Y\!\cdot\!F\!\cdot\!\mu_0$   & $930$--$953$ & $938.272$ & $-0.9$ to $+1.6\%$\footnotemark[1] \\
\bottomrule
\end{tabular}
...
($2\mu_0$, two-winding) and the proton's trefoil factor ($N_Y=3$, the crossing
...
The muon identification $m_{\mu^\pm}=(3/2)\mu_0$ is a \textbf{numerical
...
closed; the muon $(3/2)\mu_0$ is a numerical coincidence with no surviving
...
\begin{itemize}
  \item The chiral scale $\mu_0=m_e/\alpha\approx 70\,\text{MeV}$ still takes
...
first-principles derivation because $\mu_0=m_e/\alpha$ and the
...
assignment $m_{\mu^\pm}=(3/2)\mu_0$ remains an empirical coincidence with no
...
$m_p c^2 = N_Y\!\cdot\!F\!\cdot\!\mu_0$ with $N_Y\approx3.007$ and form
...
Muon mass $m_{\mu^\pm}=(3/2)\mu_0$ ($0.59\%$ from CODATA) &
...
Proton mass $m_p \approx N_Y \cdot F \cdot \mu_0 \approx 930$--$953$
...
free input: $\mu_0=m_e/\alpha$ (still empirical, the precision-bearing
test).  Records:
...
$\mu_0=m_e/\alpha$.  For the pion, $N=2$ is fixed by topology.  For the proton,
...
spectrum.  $\mu_0$ is not independently predicted; it is $m_e/\alpha$.
...
denominator $\mu_0^{\rm straight}(R)$ carries the cutoff, so the ratio is
...
\begin{figure}[h]
  ...
  \caption{Electron vortex ring energy landscape.
    ...
    electron radius.  Computed with $\alpha = 1/137.036$.}
...
motivated the muon identification $m_{\mu^\pm}\approx(3/2)\mu_0$, together
