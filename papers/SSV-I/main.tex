...
\textit{ansatz}, and links each candidate-grade claim to the specific
GitHub issue (\#13, \#16) whose computation would change its status.
...
the observed value $0.207$ and narrows as $L_{\rm box}$ grows, but does not
yet resolve to a single number; a bracket consistent with the target is a
weaker claim than a sharp prediction, and we state it as such.  Higher rungs
of the ladder reach sub-percent accuracy in the same projection, consistent
with the expectation that core-localised modes are well represented by a
meridional slice while ring-scale modes (the muon) require either thin-ring
analytic asymptotics or a full 3D grid that spans the entire circumference
$2\pi R_e=2\pi\xi/\alpha$.  Both refinements are deferred to Paper~II; the
thin-ring matrix-element calculation is the falsification test for the muon
identification.

\textbf{Status of the residual gap.}  At the matched-spacing reference point
$(n=59,\,L_{\rm box}=6\,\xi,\,\phi\text{-grid}=128)$ in the 10-mode
combined-Kelvin / weak-form basis, the converged
eigenfrequency is
$\omega_\mu/\omega_c\approx0.2073$, a $0.15\%$ overshoot of the CODATA
target $0.20700$ (numerical record:
\texttt{papers/SSV-I/data/} matched-spacing trio).  The structural cubic-vertex
one-loop self-energy is computed in the repository
(\texttt{src/paper\_i/muon\_cubic\_full\_self\_energy.py}) and gives a
relative shift of $-(3\text{--}9)\times10^{-9}$ at three grid resolutions
($n=25,31,49$) -- seven orders of magnitude too small and the wrong sign to
close the residual gap.  The cubic vertex is therefore falsified as the
source of the gap; the residual is consistent with a Galerkin truncation of
the reduced eigenvalue problem and scales as $1/\sqrt{N_{\rm modes}}$ across
the electron, pion, muon, and proton calculations
(see \texttt{papers/SSV-I/topology-scaling-of-residual-memo.md} and
\texttt{papers/SSV-I/muon-cubic-vertex-result-note.md}).  A first
enrichment-direction attempt at $N_{\rm modes}\!=\!15$ produced an
inconsistent spectrum due to a cross-$m$ current-curl issue in the basis
construction documented in
\texttt{papers/SSV-I/enrichment-attempt-findings-2026-05-27.md};
the corrected enrichment programme is open work tracked under
GitHub issue~\#16.
...
\begin{equation}
  ...
  \label{eq:muon-mass}
\end{equation}
is a dynamically motivated rung (status: \textit{candidate};
\S\ref{sec:claim-status}) with an identified mechanism (core-breathing
$\oplus$ Kelvin coupling through $\mathcal{L}_\perp$), a verified selection
rule (driven-response scans), a numerical eigenfrequency bracket that
contains the target value, and an identified residual gap that has been
falsified against the cubic-vertex hypothesis and reframed as a
topology-scaled basis-truncation error.  The thin-ring analytic closure
deferred to Paper~II would turn this candidate into a falsifiable single
number; the basis-enrichment programme (issue~\#16) would independently
demonstrate the $1/\sqrt{N_{\rm modes}}$ scaling claimed here.
...
\end{equation}
with $N_Y\approx3.007$ (topological crossing factor, candidate status,
...
(Appendix~\ref{app:F-extraction}).  $F$ depends on the straight-vortex
calibration cutoff $R$: at $R=1.18\,\xi$ (the inter-strand half-spacing of
the trefoil) the fine-grid average $(n^3{=}48^3,\,72^3)$ gives
$F\approx4.47$, agreeing between the two grids at the $\sim6\%$ level
(the raw numerator $e_{\rm interior}$ is the limiting factor; coarser
$n=24$ at the same cutoff gives $F\approx4.55$, and the n=24 numerator
differs from the fine-grid limit by ${\sim}54\%$ at the original
unrescaled cutoff).  The two fine grids give
$m_p \approx 930$--$954\,\text{MeV}$ at $R=1.18\,\xi$; at $R=1.5\,\xi$
the same observable falls to ${\sim}760\,\text{MeV}$ ($-19\%$).  The
logarithmic slope is $\mathrm{d}\ln F/\mathrm{d}\ln R\approx-0.94$ near
$R=1.18\,\xi$, so a $\pm0.3\,\xi$ uncertainty on the cutoff
propagates as $\sim3\%$ on $F$ alone.

The honest framework prediction is therefore
$m_p \approx 930$--$954\,\text{MeV}$ (within ${\sim}1.5\%$ of the CODATA
value 938.272 MeV) at the canonical cutoff $R\approx1.18\,\xi$, with a
documented cutoff systematic and a not-yet-measured cross-grid spread on
$N_Y$ itself.  The single-number $0.3\%$ figure that appeared in earlier
drafts of this paper is the precision at that specific cutoff, not a
cutoff-independent first-principles prediction; promoting it requires
either deriving the cutoff geometrically from the relaxed breather or
demonstrating that $N_Y\cdot F$ is itself cutoff-invariant.  Both are
deferred to the closure work tracked under GitHub issue~\#13 and listed
in \S\ref{sec:claim-status}.
...
\bottomrule
\end{tabular}
\footnotetext[1]{$F\approx4.47$ at the inter-strand half-spacing cutoff
$R=1.18\,\xi$ (mean of $n^3{=}48^3$ and $72^3$ straight-vortex extractions;
those two grids agree at the $\sim6\%$ level on this observable).  The
range $930$--$954\,\text{MeV}$ spans the two fine grids.
$F$ varies as $\mathrm{d}\ln F/\mathrm{d}\ln R\approx-0.94$ in this region;
the cutoff is physically motivated but not yet derived, giving a ${\sim}3\%$
systematic per $0.3\,\xi$ uncertainty.  Status: candidate
(\S\ref{sec:claim-status}, GitHub issue \#13).}
\end{table}
...
status label and a pointer to the GitHub computation issue whose
resolution would change that status.  The taxonomy follows
...
\toprule
Claim & Status & Upgrade condition & Issue \\
\midrule
...
condition.  No separate dynamical confirmation yet. & \#16 \\
...
status one step toward derived. & \#13 \\
...
\texttt{src/paper\_i/topology\_penalty.py}. & \#13 \\
...
should not be quoted as derived. & \#16 \\
...
not currently a limitation downstream. & \#16 \\
...
\begin{itemize}
  ...
  \item the $N_Y$ convergence statement ``independent of grid resolution
    ...
    sensitivity check is listed as outstanding work under issue~\#13.
\end{itemize}
...
(closure work tracked under GitHub issue~\#13).
...
(as opposed to $F$) is part of the open closure work tracked under GitHub
issue~\#13 and \S\ref{sec:claim-status}.  A practical implementation relaxes
...
spread for $N_Y$; see GitHub issue~\#13.
