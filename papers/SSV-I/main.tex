...
\begin{gapbox}[Status (2026-05): muon eigenmode not basis-converged]
The toroidal-breather eigenvalue test (``Path~B'') applied to the operator
constructed in this section was pre-registered and run; full record in
\texttt{papers/SSV-I/path-b-eigenvalue-result.md}.  Outcome: at the claimed
parameter-free coupling $\lambda_\perp=\pi/4$, the muon hits
$\omega/\omega_c=0.207$ in the single published basis (\texttt{helicity}
Kelvin seed) and within $3.7\%$ at face value (within $0.09\%$ in the
relaxed-background variant of Appendix~\ref{app:helicity-bridge}), but the
lowest physical mode drifts $\pm 13\%$ across four basis choices and leaves
the muon window empty in two of them.  The charged-pion rung $2\mu_0$ is
empty in every basis tested.  The spectrum is not evenly spaced.  The muon
identification therefore remains a candidate carried by the empirical
agreement on the $m_e/\alpha$ scale, not by a basis-converged eigenfrequency.
The mechanical text below is retained as the structural setup; readers should
treat its claims of ``convergence'', ``selection rule'', and ``residual gap''
in the light of this null.  Tracking: issue~\#66.
\end{gapbox}
...
agreement on the $m_e/\alpha$ scale rather than by a basis-converged
eigenmode.  The status is therefore weaker than the previous draft stated:
the ``identified mechanism'' (core-breathing $\oplus$ Kelvin via
$\mathcal{L}_\perp$) is the \emph{target} of the open derivation, not its
result; the ``verified selection rule'' from driven-response scans does not
survive basis enrichment; and the ``numerical eigenfrequency bracket'' is
basis variation, not Galerkin convergence.  The thin-ring analytic closure
deferred to Paper~II and the basis-enrichment programme (issue~\#66) are
now the two routes that would convert this candidate into either a sharp
