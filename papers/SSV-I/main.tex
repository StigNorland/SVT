...
coincidence & \textbf{Demoted 2026-06} from candidate to numerical
coincidence: five probes (Path~B, \#73, \#76, App.~B potential,
two-mode reduction) converge on a no-go --- a scalar U(1) order
parameter has no half-integer Berry holonomy and the
$\mathcal{L}_\perp$ bridge vanishes by selection rules
(\texttt{muon-bdg-nogo-summary.md}).  The BdG-breathing programme is
retired.  Upgrade requires Route~C (generations as distinct static
minima) or new microscopic structure (spinor / half-winding sector). &
\#66,\#73,\#76,\#78 \\
