...
muon therefore remains a candidate empirical rung rather than a derived
eigenfrequency.  For the proton, the empirical scaling
...
$0.3\%$ figure that appeared in earlier drafts.  \textbf{Update (issue~\#92,
2026-06):} the node factor $N_Y$ is now \emph{derived} --- it is the trefoil
crossing number $N_Y=3$ (equivalently the thin-core writhe, or the braid-word
exponent $\sigma_1^3$); the $3.007$ is this $3$ plus a small finite-thickness
writhe correction, and the alternative $l_{\rm curve}/(4\pi)$ proxy is rejected
as non-invariant (it floats $2.0$--$3.1$ with geometry).  The cutoff $R$ is the
inter-strand spacing ($=2a$, a geometric property of the relaxed knot, set by
the energy-minimum $a\approx0.85\,\xi$), so route~(i) holds in principle;
route~(ii) ($N_Y\!\cdot\!F$ cutoff-invariant) was falsified (\#77).  A
$\sim 6\%$ $F$-systematic remains from the shallow geometric minimum, and
$\mu_0=m_e/\alpha$ stays empirical.  Full record:
\texttt{results/proton/ny-trefoil-derivation-result.md}.  Tracked under
issues~\#66, \#92.
\end{gapbox}
...
\textbf{Update (issue~\#92, 2026-06): $N_Y$ is now derived.}  $N_Y$ is the
...
\begin{itemize}
  ...
  \item A residual $\sim 6\%$ systematic in $F$ from the shallow geometric
        minimum (the cutoff $R=$ inter-strand spacing $=2a$ is geometric, but
        $\mathrm{d}\ln F/\mathrm{d}\ln R\approx-0.94$ propagates the
        $a$-uncertainty into $F$).
\end{itemize}
\end{gapbox}
...
proton assignment is a candidate $1$--$2\%$ estimate rather than a closed
derivation; later issue~\#30 closure gates show that the current static
pipeline is not topology-robust or cutoff-derived.  The pion is on stronger
footing: its two-winding topology is topologically forced, and the remaining
gap is the stability proof for $L=2R_e$.  The muon remains a candidate
empirical rung after the Path~B, issue~\#73, and issue~\#76 null results.
...
candidate & \textbf{Update (\#92):} the node factor is now derived,
$N_Y=3$ (crossing number; see row below).  The cutoff $R$ is the
geometric inter-strand spacing ($=2a$, set by the energy-minimum
$a\approx0.85\,\xi$), so the cutoff is not free; a $\sim6\%$ $F$-systematic
remains from the shallow geometric minimum ($\mathrm{d}\ln F/\mathrm{d}\ln
R\approx-0.94$).  Cutoff-invariance route falsified (\#77).  Remaining
free input: $\mu_0=m_e/\alpha$ (still empirical, the precision-bearing
test).  $m_p=3\times4.4\times70\approx924$\,MeV.
\texttt{results/proton/ny-trefoil-derivation-result.md}. & \#30,\#92 \\
...
on $n^3{=}48^3,\,72^3$ agrees at $\sim6\%$ at this
observable; the no-penalty value ($F\approx4.47$) is from
topologically trivial states.  Penalty-valid $F\in[10,\,17]$
is not yet grid-converged.  Three cutoff approaches
(cutoff-invariance, geometric-$R$, self-consistent $R_{\rm sc}$)
have been tested and all give large spread.  See \#30 for active
$n=96$ cross-grid run. & \#30 \\
