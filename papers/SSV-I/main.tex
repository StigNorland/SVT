...
node.  Those presuppose the junction and are therefore \textbf{unsupported as
written}.  They are retained below as a record of the original reasoning, not
as derived results.  This converges with issue~\ssvissue{178}, which found by a
fully independent route ($\pi_3(S^1)=0$) that the bare one-component theory has
no fermionic solitons: both point at \textbf{multi-component structure} as
missing content rather than as a patch.
\end{resultbox}
...
\textbf{unsupported} & Presupposes the withdrawn Y-junction.  Requires
multi-component structure (three components with coherent coupling and domain
walls in the relative phases).  Converges with \ssvissue{178}
($\pi_3(S^1)=0$, no fermionic solitons in the bare theory) on the same
repair. \\
