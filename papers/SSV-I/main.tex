...
\begin{gapbox}[Open Problem 2 — Muon: current scalar-BdG closure paths failed]
The muon identification $m_{\mu^\pm}=(3/2)\mu_0$ remains a candidate
assignment carried by the empirical $m_e/\alpha$ scale, not a derived
eigenfrequency.  Three closure attempts now give null results.  The
...
\begin{itemize}
  ...
  \item the muon identification has been tested by three independent
    null results (Path~B, issue~\#73, issue~\#76 Berry-phase audit).
    No half-integer Berry holonomy exists in the current scalar SSV
    BdG operator.  The muon and higher rungs remain candidate-grade.
\end{itemize}
