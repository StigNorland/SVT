...
candidate rung; post-\ssvissue{78} lepton-generation work requires a new HQV or
spinorial branch.  For the proton, the candidate static-geometry scaling
...
\begin{itemize}
  \item \textbf{Spin.}
        ...
        \texttt{papers/SSV-I/results/spinor/b2-lepton-spin-half.md}.)
...
\texttt{results/proton/ny-trefoil-derivation-result.md} and
\texttt{results/proton/geometry-minimum-result-2026-06-03.md}.  Tracked under
...
\item \textbf{Path~B (BdG eigenvalue):} the muon and pion rungs were not
  ...
  Full record: \texttt{papers/SSV-I/results/muon/path-b-eigenvalue-result.md}.
\item \textbf{Berry-phase audit (issue~\ssvissue{76}):} Volovik's CdGM framework was
  ...
  \texttt{papers/SSV-I/results/muon/volovik-berry-phase-issue76-tasks1-4.md}.
\end{itemize}
...
\texttt{results/muon/muon-bdg-nogo-summary.md}.
...
\item \textbf{Issue~\ssvissue{73} (phi-discretised BdG):} a direct $m_\varphi=\pm1$
  ...
  Full record: \texttt{papers/SSV-I/results/muon/muon-issue73-phi-bdg-result.md}.
...
\texttt{papers/SSV-I/results/muon/path-b-eigenvalue-result.md}) shows that the muon
...
\texttt{papers/SSV-I/results/muon/path-b-eigenvalue-result.md}).  The thin-ring
...
\texttt{instruments/\_fitted\_quarantine/paper\_i/muon\_cubic\_full\_self\_energy.py}
...
\texttt{instruments/paper\_i/trefoil\_ny\_derivation.py}.  What remains open:
\begin{itemize}
...
$\mathbb{Z}_2$ for a scalar field. & \ssvissue{29},\ssvissue{83},\ssvissue{87} \\
...
vanishes by selection rules, \texttt{muon-bdg-nogo-summary.md}).
...
final.} Full records: \texttt{results/spinor/b1-winding-regime-result.md},
\texttt{hqv-muon-verdict.md}. & \ssvissue{76},\ssvissue{87},\ssvissue{91},\ssvissue{94} \\
...
Recorded as a numerical coincidence, not a candidate rung. & \ssvissue{76},\ssvissue{91},\ssvissue{93} \\
...
test).  Records:
\texttt{results/proton/ny-trefoil-derivation-result.md};
\texttt{results/proton/geometry-minimum-result-2026-06-03.md}. & \ssvissue{77},\ssvissue{92},\ssvissue{98} \\
...
\texttt{results/proton/ny-trefoil-derivation-result.md}. & \ssvissue{92} \\
...
documented $Q_p/\alpha_G$ map. & \ssvissue{77},\ssvissue{98} \\
...
should not be quoted as derived. & closed \ssvissue{30} \\
...
Record: \texttt{papers/SSV-I/results/mu0-parameter-count-result.md}.
...
\begin{itemize}
  ...
  \item three cutoff routes for $N_Y\cdot F$ have been tested and all
    ...
    (\texttt{static-closure-self-consistent-r-result.md}).
...
\texttt{instruments/paper\_i/topology\_penalty.py}).  Vortex lines are tracked where
...
\texttt{trefoil\_breather\_static.py}, and the topology-preserving 3D
minimisation under the full $\mathrm{LogSE}+\mathcal{L}_\perp$ functional is
performed by \texttt{trefoil\_gradient\_flow\_static.py} (pure imaginary-time
...
(\texttt{trefoil\_breather\_observables.py}) --- not by a 1D filament line
integral.
...
of \texttt{instruments/paper\_i/topology\_penalty.py}, and the cutoff-stability
...
grid (\texttt{trefoil\_gradient\_flow\_static.py}), and the node-cost factor is
extracted by \texttt{trefoil\_breather\_observables.py} as the energy in a
...
(\texttt{instruments/paper\_i/topology\_penalty.py}) converges to
...
tree at \texttt{instruments/paper\_i/trefoil\_breather\_lperp\_krylov\_static.py}
and \texttt{instruments/paper\_i/trefoil\_breather\_observables.py}, with all
...
Verified in \texttt{instruments/paper\_i/trefoil\_ny\_derivation.py};
record \texttt{results/proton/ny-trefoil-derivation-result.md}.

Full code, saved states ($\sim\!100$\,MB across all grids),
density-slice plots, and sensitivity analyses are catalogued in
\texttt{papers/SSV-I/results/proton/proton-mass-final-checkpoint.md} and the
