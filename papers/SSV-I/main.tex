...
\end{itemize}
As of 2026-06 the five probes above have converged on a single no-go: a scalar
U(1) order parameter admits no half-integer Berry holonomy, and the
$\mathcal{L}_\perp$ bridge that could generate one vanishes by the azimuthal
selection rules.  Consolidated record:
\texttt{papers/SSV-I/results/muon/muon-bdg-nogo-summary.md}.  Consequently the muon rung
$(3/2)\mu_0$ is \textbf{demoted from candidate rung to numerical coincidence}
pending Route~C (generations as distinct static minima, issue~\#78): it matches
the PDG mass to $0.59\%$, but the present scalar operator supplies no mechanism
that selects $3/2$.  The pion rung ($n=2$, integer) is unaffected --- it lives
in the integer sector the pure-LogSE holonomy does support.  A positive muon
derivation would require new microscopic structure (spinorial order parameter,
or a derived half-winding Nambu mass), not another pass through the reduced
basis.
\end{gapbox}
...
\emph{What remains}: a positive muon derivation requires a changed
microscopic ingredient not present in the current scalar formulation, such
as a spinorial order parameter, a derived half-winding Nambu mass, or a
future full 3D operator whose eigenbundle genuinely carries holonomy $-1$.
The existing helicity-bridge numerics are retained as historical diagnostics
but no longer close the rung.
\end{gapbox}
...
topological-sector (cond.) & \textbf{Demoted 2026-06} from candidate to
...
parameter (no half-integer Berry holonomy; the $\mathcal{L}_\perp$ bridge
vanishes by selection rules, \texttt{muon-bdg-nogo-summary.md}).
\textbf{Re-promoted 2026-06 (\#87 B1)} under the CP$^1$/spinor extension
(\#83): the spin--orbit-locked spinor BdG operator gives a protected
half-integer Berry phase $\gamma_B=\pi$ (a $\mathbb{Z}_2$ invariant of the
Nambu eigenbundle), so $E_n=(n+\tfrac12)\omega_0$ and the muon is the
$3/2$ rung --- TOPOLOGICAL-SECTOR.  Conditional on showing the physical
torus eigenbundle is in the winding regime (\texttt{results/spinor/b1-
muon-berry-phase.md}). & \#66,\#76,\#78,\#87 \\
