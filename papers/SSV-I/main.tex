...
\begin{gapbox}[Status (2026-05): empirical coincidence, not a derived spectrum]
The pattern above is real, but it is carried by very few rungs.  A
pre-registered re-analysis of the 14-particle table (Path~A) found that
removing either the muon ($3/2$) or the charged pion ($2$) collapses the
specialness of $\mu_0$ from top-$3\%$ to top-$10$-$11\%$ of nearby scales;
the other twelve rows do not independently support the rung structure.  A
pre-registered eigenvalue test (Path~B) then attempted to obtain the muon
and pion rungs as genuine eigenfrequencies of the
$\mathcal{L}+\mathcal{L}_\perp$ toroidal-breather BdG operator and failed
its own decision rule: the muon reaches $\omega/\omega_c=0.207$ only in one
truncated basis and drifts $\pm13\%$ (or vanishes) under Galerkin
enrichment, the pion rung is absent in every basis tested, and the
resulting spectrum is not evenly spaced.  The two sub-percent rungs
($\mu^\pm,\pi^\pm$) should therefore be read as a two-coincidence numerology
in $m_e/\alpha$, pending a basis-converged eigenvalue calculation.  Full
record: \texttt{papers/SSV-I/path-b-eigenvalue-result.md}.
\end{gapbox}
...
number; the basis-enrichment programme (issue~\#30) would independently
...
deferred to the closure work tracked under issue~\#30 and listed
...
(\S\ref{sec:claim-status}, issue~\#30).}
\end{table}
