...
fixed by $\Delta r$.  The current repository receipt in Paper~VI-a fits the
Chemin et al.~\cite{chemin2009} Table~4 H\,I rotation curve directly, without a 2D
beam-convolution model, and obtains RMS $\approx9.27\;\mathrm{km/s}$ for
$r\ge4$~kpc.  A closure-grade beam-convolved 2D fit remains to be added.

\begin{gapbox}[Beam-convolved receipt needed]
The axisymmetric receipt from Paper~VI-a supports the radial Bessel term, but the
beam-convolved two-dimensional velocity-field comparison has not yet been reproduced as a
live repository calculation.  The figure below should therefore be read as a morphology
schematic until the projected-disc fit is rebuilt with the Chemin et al. data cube or an
equivalent public velocity-field product.
\end{gapbox}

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig1_rotation_curve.jpg}
\caption{NGC~224 rotation-curve schematic.  \emph{Blue}: SSV model~\eqref{eq:vmodel}.
\emph{Red dashed}: smooth halo-style comparison.
\emph{Dotted verticals}: predicted Bessel node positions, not fitted.
\emph{Lower panel}: residuals.  A closure-grade beam-convolved reproduction is deferred.}
\label{fig:rc}
\end{figure}
