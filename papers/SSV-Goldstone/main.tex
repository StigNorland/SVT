...
This paper isolates the candidate correction suggested by that audit.  The
saturated order parameter has a longitudinal Goldstone/Bogoliubov sound mode
at $c$, but a scalar phase mode alone is not a photon: it supplies neither two
transverse polarizations nor Maxwell gauge structure.  The chiral-shear
sector remains physically real, but it is provisionally reassigned
...
order.  This eliminates the proposed slow chiral-shear photon, but it does
not by elimination establish the remaining scalar sound mode as the
electromagnetic photon.  The internal CP$^1$ direction is quadratic and
charge-decoupled at linear order, so its transport cannot simply be asserted
to provide two photon polarizations.  Deriving a transverse, gauge-like
two-polarization sector with displacement-current coefficient $1/c^2$ is
