...
These were the original seed issues. Most have since been opened and closed;
this file now records what remains worth opening next.

## Paper tasks

- [x] Rebuild `SSV III` around irreversible time and wake entropy
- [x] Split current `SSV V` into `SSV VII-a`, `SSV VII-b`, and `SSV VIII`
...
- [x] Build one shared numerical conventions note for `\mathcal{L}+\mathcal{L}_\perp`
- [x] Turn the trefoil breather into a real 3D static minimisation task with diagnostics
...
- [x] Mark proton `N_Y` and `F` as provisional during the pre-#77 static-pipeline phase
- [x] Promote only the #77 product `N_Y×F=54` to candidate static geometry, not separate closure-grade `N_Y` and `F`
- [x] Mark `W` mass as an ansatz-backed estimate until cap geometry is measured directly
- [x] Mark neutrino, PMNS, and CP claims as downstream targets of the reconnection spectrum
- [x] Mark scalar lepton generation routes C/D as failed after #78

## Objection tasks

- [x] Write a clean answer to "If time stops at the horizon, why does the black hole spin?"
- [x] Write a clean answer to "Why is time reversal in equations not the same as physical reversal?"
