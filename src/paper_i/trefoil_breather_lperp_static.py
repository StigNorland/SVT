"""(2,3)-trefoil knot static prototype with the L_perp chiral non-local shear term.

Status: prototype
Problem type: static
Nondimensionalisation: xi = 1, background density rho0 = 1, longitudinal speed c = 1
Primary observables: total energy (LogSE + L_perp), residual norm, depressed_fraction, min_density
Primary role: tests whether the L_perp term from Paper I (eq. `chiral`) prevents
              the curvature-driven dissolution of the closed (2, 3)-trefoil knot
              under pure LogSE gradient flow that we documented at lambda = 0.
              This is the binary "does L_perp stabilise closed vortex topology"
              experiment.
Key limitation: explicit Euler is stiff at large lambda; the paper's quoted
                lambda = 2000 is well beyond reach at dx = 0.5.  This script
                accepts lambda as a CLI parameter for ramping.
"""
...
SCRIPT_METADATA = ScriptMetadata(
    ...
    issue_refs=("#13",),
    limitations=(
        "Explicit Euler gradient flow is stiff at large lambda; paper's lambda = 2000 needs implicit methods or much finer grid.",
        "Uses the (2,3) torus-knot initial condition from trefoil_breather_static; topology choice is inherited.",
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="(2,3)-trefoil knot relaxation with L_perp chiral non-local shear term.")
